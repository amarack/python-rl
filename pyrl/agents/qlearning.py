...
class qlearning_agent(Agent):
	...
	def agent_step(self,reward, observation):
		...
		self.update(phi_t, newState, reward)
	...
	def update(self, phi_t, state, reward):
		Q_tp = 0.0
		a_tp = 0
		s_tp = None
		if state is not None:
			...
			Q_tp = numpy.dot(self.weights.T, s_tp)
			a_tp = Q_tp.argmax()
			Q_tp = Q_tp.max()
...
if __name__=="__main__":
	...
	parser.add_argument("--tiles_num", type=int, default=10, help="Number of tilings to use with Tile Coding.")
	parser.add_argument("--tiles_size", type=int, default=1024, help="Memory size, number of weights, to use with Tile Coding.")
